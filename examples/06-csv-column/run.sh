#!/usr/bin/env sh
# Pull column 3 out of patients.csv using the built-in --column flag,
# skipping the header row. Diff against the expected files.
set -eu
here="$(cd "$(dirname "$0")" && pwd)"
bin="$here/../../target/debug/nhs-number-cli"
[ -x "$bin" ] || bin="${bin}.exe"
[ -x "$bin" ] || { echo "Binary not found at $bin — run 'cargo build' first." >&2; exit 2; }

tmp_out="$(mktemp)"; tmp_err="$(mktemp)"
trap 'rm -f "$tmp_out" "$tmp_err"' EXIT

tail -n +2 < "$here/patients.csv" | "$bin" --column 3 > "$tmp_out" 2> "$tmp_err"
diff -u "$here/expected-stdout.txt" "$tmp_out"
diff -u "$here/expected-stderr.txt" "$tmp_err"
echo "OK"
